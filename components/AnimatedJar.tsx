...
export default function AnimatedJar({ pct, tierColor, coinCount, showCoinDrop, showCoinRemove, onAnimDone }: Props) {
    ...
    // Coin remove animation (hand grabs coin)
    useEffect(() => {
        if (showCoinRemove) {
            handX.setValue(80);
            handY.setValue(20);
    ...
    return (
        <View style={s.wrap}>
            ...
            <Svg width={160} height={240} viewBox="0 0 160 240" style={s.svg}>
                <Defs>
                    ...
                    <SvgGrad id="rim" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <Stop offset="1" stopColor="#888" stopOpacity="0.6" />
                    </SvgGrad>
                </Defs>
                {/* Jar body - mason jar shape */}
                <Path d="M 35 60 C 25 70, 20 90, 20 120 L 20 190 C 20 215, 35 230, 80 230 C 125 230, 140 215, 140 190 L 140 120 C 140 90, 135 70, 125 60 Z"
                    fill="url(#glass)" stroke="rgba(255,255,255,0.3)" strokeWidth="2" />
                {/* Jar neck */}
                <Rect x="40" y="38" width="80" height="22" rx="4" fill="url(#glass)" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5" />
                {/* Rim / lid */}
                <Rect x="35" y="32" width="90" height="10" rx="5" fill="url(#rim)" />
                {/* Glass highlight left */}
                <Path d="M 30 80 C 28 100, 28 140, 30 180" stroke="rgba(255,255,255,0.25)" strokeWidth="3" strokeLinecap="round" fill="none" />
                {/* Glass highlight right subtle */}
                <Path d="M 130 90 C 132 110, 132 150, 130 175" stroke="rgba(255,255,255,0.1)" strokeWidth="2" strokeLinecap="round" fill="none" />
                {/* Bottom ellipse reflection */}
                <Ellipse cx="80" cy="225" rx="40" ry="4" fill="rgba(0,0,0,0.08)" />
            </Svg>
            ...
            <Animated.View style={[s.water, { height: waterHeight, backgroundColor: tierColor + 'DD' }]}>
                ...
                {/* Bubbles */}
                <AnimatedView style={[s.bubble, { left: '30%', transform: [{ translateY: bubbleY1 }], opacity: bubbleOp1 }]} />
                <AnimatedView style={[s.bubble, s.bubbleSm, { left: '60%', transform: [{ translateY: bubbleY2 }], opacity: bubbleOp2 }]} />
            </Animated.View>

            {/* Shimmer overlay */}
            <AnimatedView style={[s.shimmer, { opacity: shimmerOp }]} />
            ...
            {/* Remove coin + hand animation */}
            {showCoinRemove && (
                <>
                    <AnimatedView style={[s.removeCoin, {
                        transform: [{ translateY: removeY }],
            ...
            {/* Percentage badge */}
            <View style={s.badge}>
                <Text style={s.badgeText}>{Math.round(pct)}%</Text>
            </View>
        </View>
    );
}
...
const s = StyleSheet.create({
    ...
    waterSurface: {
        height: 6, width: '100%', borderRadius: 3, opacity: 0.7,
    },
    coinInJar: { position: 'absolute' },
    coinEmoji: { fontSize: 16 },
    bubble: {
        position: 'absolute', bottom: 10, width: 6, height: 6,
        borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.5)',
    },
    bubbleSm: { width: 4, height: 4, borderRadius: 2 },
    shimmer: {
        position: 'absolute', top: 65, left: 30, width: 4, height: 120,
    ...
    badge: {
        ...
        backgroundColor: '#FFF', paddingHorizontal: 16, paddingVertical: 6,
        borderRadius: 20, elevation: 6,
        shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 10, shadowOffset: { width: 0, height: 4 },
    },
    badgeText: { fontWeight: '900', fontSize: 16, color: '#1a1a2e' },
});
